<script setup>
    ...
    const deleteUser = () => {
        ...
        .then(() => {
            let usersData = users.value.data;
            $('#deleteUserModal').modal('hide');
            Swal.fire('success', 'Delete user successfully!', 'success');
            // Lọc danh sách người dùng để loại bỏ người dùng bị xóa
            usersData = usersData.filter(user => user.id !== userIdDeleted.value);
            // Gán danh sách người dùng đã lọc lại cho users.value.data
            users.value.data = usersData;
        });
    };
